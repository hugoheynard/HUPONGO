...
class Racket extends GameObjects {
    ...
    bonusCatcher() { // console log A SUPPRIMER 
        ...
        .forEach(item => {
            if ((item.coordinates[1] >=  this.coordinates[1]) && (item.coordinates[1] <=  this.coordinates[1] + this.height)) {
...
class Ball extends GameObjects {
    ...
    set lastGoal(newGoal) {
        this._lastGoal = newGoal;
    }
    ...
    collisionManager() {
        //logic : if I meet another element in the GameObjectList : 
        GameObjects.getItemList().forEach(item => {
            //If the meeting happens between Y bottom and Y top, we check X, if item is collider, manage collision.
        ...
        const timeInterval = setInterval (() => {
            ...
            if (this.isMoving) {
                if(this.movingDirection === 'movingUp') {
                    this.coordinates[1] += this.movingSpeed;
            ...
            if ((ball.isMoving === false) || ((this.coordinates[1] < -40) || (this.coordinates[1] > 1040))) {
                this.unDisplay(this);
...
const startingPosition = () => {
    if (ball.lastGoal === 'player') {
        ball.coordinates = [495, 20];
    } else if (ball.lastGoal === 'computer') {
        computer.coordinates = [420, 980]
        ball.coordinates = [495, 960];
        ball.findBouncingAngles(computer);
        setTimeout(() => {
            ball.play();
            document.getElementById("pong").play();
        },1000);
